...
Lymphocyte.prototype = {
	...
	moveTo: function (networkOutput) {
		...
		var separation = this.separate(this.world.lymphocytes);
		var alignment = this.align(this.world.lymphocytes).setAngle(angle);
...
Bacteria.prototype = {
	...
	moveTo: function (networkOutput) {
		...
		var target = new Vector(networkOutput[0] * this.world.width, networkOutput[1] * this.world.height);
		var angle = (networkOutput[2] * this.HALF_PI) - Math.PI;

		var separation = this.separate(this.world.bacterias);
		var alignment = this.align(this.world.bacterias).setAngle(angle);
		var cohesion = this.seek(target);
	...
	draw: function () {
		...
		x2 = this.location.x + Math.cos(angle + this.QUARTER_PI) * this.base;
		y2 = this.location.y + Math.sin(angle + this.QUARTER_PI) * this.base;

		x3 = this.location.x + Math.cos(angle - this.QUARTER_PI) * this.base;
		y3 = this.location.y + Math.sin(angle - this.QUARTER_PI) * this.base;
	...
	boundaries: function () {
		...
		if (this.location.x > this.world.width - 20)
			this.applyForce(new Vector(-this.maxforce * 2, 0));

		if (this.location.y < 20)
			this.applyForce(new Vector(0, this.maxforce * 2));

		if (this.location.y > this.world.height - 20)
			this.applyForce(new Vector(0, -this.maxforce * 2));
